...
public class ControlPanelRenderer extends BaseBlockRenderer<Block> {
	...
	static AxisAlignedBB panelBox = AxisAlignedBB.getBoundingBox(
		-0.5, -h - d1, -0.5,   0.5, h + d1, 0.5);
		
	static Matrix3 leverRotations[] = {
		Matrix3.rotX(45), Matrix3.rotX(-45)
	};
...
//		System.out.printf("ControlPanelRenderer.renderHighlight: finished\n");
	...
	void setup(ControlPanelMaterial base) {
		if (base != null)
			setup(base.block, base.metadata);
		else
			setup(Blocks.planks, 0);
	}
...
//				x, y, z);
	...
	void renderBackCutouts(Trans3 t) {
		...
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++)
				face(t, -0.5 + i * 0.25, h + d2, -0.5 + j * 0.25,
					0, 0, 0.25,   0.25, 0, 0,   0, 0, 1, 1);
	}
		...
		switch (phase) {
			case STATIC:
				if (meta == 16)
					...
					icon = Blocks.wool.getIcon(0, meta & 0xf);
	...
	void pbtBox(Trans3 t, AxisAlignedBB box) {
		pbtBox(t, box.minX, box.minY, box.minZ,
	...
	void pbtBoxFace(Trans3 t, int side,
...
//			System.out.printf("ContolPanelRenderer.ptRect\n");
	...
	void ptVertex(Trans3 t, double x, double y, double z) {
		...
		switch (globalSide) {
			case 0:                                                  // DOWN
			case 1: u =  p.x - blockX; v =  p.z - blockZ; break;     // UP
			case 2:                                                  // NORTH
			case 3: u =  p.x - blockX; v = -p.y + blockY + 1; break; // SOUTH
			case 4:                                                  // WEST
			case 5: u =  p.z - blockZ; v = -p.y + blockY + 1; break; // EAST
		}
//		if (debugLightVertex)
//			System.out.printf(
...
//	}
	...
	void lightVertex(Trans3 t, double x, double y, double z) {
		if (world != null) {
			...
			int br = block.getMixedBrightnessForBlock(world,
