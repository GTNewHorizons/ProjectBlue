...
public class ControlPanelRecipes {
    ...
    public static ItemStack miniatureStackFor(ItemStack stack) {
        ...
        if (item instanceof ItemBlock) {
            ...
            if (block == Blocks.lever) return new ItemStack(ProjectBlue.miniatureLever, 1, 16);
            if (block == Blocks.stone_button) return new ItemStack(ProjectBlue.miniatureButton, 1, 16);
            if (block == Blocks.wooden_button) return new ItemStack(ProjectBlue.miniatureButton, 1, 17);
        }
        if (item == ProjectBlue.itemPartFixture) return new ItemStack(ProjectBlue.miniatureLamp, 1, meta);
    ...
    public static ItemStack fullSizeStackFor(ItemStack stack) {
        Item item = stack.getItem();
        int meta = stack.getItemDamage();
        if (item == ProjectBlue.miniatureLever && meta == 16) return new ItemStack(Blocks.lever);
        if (item == ProjectBlue.miniatureButton && meta == 16) return new ItemStack(Blocks.stone_button);
        if (item == ProjectBlue.miniatureButton && meta == 17) return new ItemStack(Blocks.wooden_button);
        if (item == ProjectBlue.miniatureLamp) return new ItemStack(ProjectBlue.itemPartFixture, 1, meta);
    ...
    static class CraftControlPanel extends RecipeBase {
        ...
        public boolean matches(InventoryCrafting ic, World world) {
            return isSaw(ic, 0, 0) && isEmpty(ic, 0, 1) && isEmpty(ic, 1, 0) && isHollowCover(ic, 1, 1);
        }
        ...
        @Override
        public void addCraftingToNEI(INEIRecipeHandler h, ItemStack result) {
            if (isControlPanel(result)) addRecipeToNEI(h, null, ControlPanelMaterial.forStack(result));
        }

        @Override
        public void addUsageToNEI(INEIRecipeHandler h, ItemStack ingredient) {
            if (isSaw(ingredient)) addRecipeToNEI(h, ingredient, ControlPanelMaterial.forName("tile.wood"));
            else if (isHollowCover(ingredient)) addRecipeToNEI(h, null, ControlPanelMaterial.forStack(ingredient));
        }
    ...
    static class ResizeControlPanel extends RecipeBase {
        ...
        @Override
        public boolean matches(InventoryCrafting ic, World world) {
            if (!isSaw(ic, 0, 0)) return false;
            ItemStack panel = ic.getStackInRowAndColumn(1, 1);
            if (panel == null || !isControlPanel(panel)) return false;
            for (int row = 0; row < 3; row++) {
                for (int col = 0; col < 3; col++) {
                    if (row == 0 && col == 0) continue;
                    if (row == 1 && col == 1) continue;
                    if (ic.getStackInRowAndColumn(col, row) != null) return false;
                }
            }
            return true;
        }
        ...
        private static int getNextSize(int current) {
            if (current == 4) return 3;
            if (current == 3) return 2;
            return 4;
        }

        private static int getPrevSize(int current) {
        ...
        public void addCraftingToNEI(INEIRecipeHandler h, ItemStack result) {
            if (isControlPanel(result)) {
                ...
                int inputSize = getPrevSize(resultSize);
        ...
        public void addUsageToNEI(INEIRecipeHandler h, ItemStack ingredient) {
            if (isSaw(ingredient)) {
                ...
                ItemStack input = wood.newStack();
                input.getTagCompound().setInteger("gridSize", 4);
                ItemStack output = input.copy();
                output.getTagCompound().setInteger("gridSize", 3);
                addRecipeToNEI(h, ingredient, input, output);
    ...
    static class CraftMiniatureItem extends RecipeBase {
        ...
        @Override
        public boolean matches(InventoryCrafting ic, World world) {
            return isSaw(ic, 0, 0) && isEmpty(ic, 0, 1) && isEmpty(ic, 1, 0) && isFullSizeControl(ic, 1, 1);
        }

        @Override
        public ItemStack getCraftingResult(InventoryCrafting ic) {
            ItemStack stack = miniatureStackFor(ic, 1, 1);
            if (stack != null) stack.stackSize = 8;
            return stack;
        }
        ...
        public void addCraftingToNEI(INEIRecipeHandler h, ItemStack result) {
            ...
            if (ingredient != null) addRecipeToNEI(h, null, ingredient);
        }
        ...
        public void addUsageToNEI(INEIRecipeHandler h, ItemStack ingredient) {
            ...
            } else if (isFullSizeControl(ingredient)) addRecipeToNEI(h, null, ingredient);
        }
    ...
    static boolean isPaintableControl(ItemStack stack) {
        if (stack != null) {
            Item item = stack.getItem();
            if (item instanceof ControlItem) switch (((ControlItem) item).type) {
                case LEVER:
                case BUTTON:
                    return true;
            }
    ...
    static boolean isSprayCan(ItemStack stack) {
        if (stack != null) return stack.getItem() instanceof SprayCanItem;
        return false;
    }
    ...
    static class PaintControl extends RecipeBase {
        ...
        @Override
        public boolean matches(InventoryCrafting ic, World world) {
            return isPaintableControl(ic, 0, 0) && isSprayCan(ic, 1, 0);
        }
    ...
    public static void registerRecipes() {
        ...
        addRecipe(new ResizeControlPanel());
