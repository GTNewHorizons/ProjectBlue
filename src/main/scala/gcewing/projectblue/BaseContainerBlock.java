...
// import static gcewing.projectblue.BaseIcons.*;
...
public class BaseContainerBlock<TE extends TileEntity> extends BlockContainer implements BaseMod.IBlock {
    ...
    public static IIcon[] getIcons(Block block, IIconRegister reg, String... names) {
        ...
        for (int i = 0; i < names.length; i++) result[i] = getIcon(block, reg, names[i]);
        return result;
    }
    ...
    public void registerBlockIcons(IIconRegister reg) {
        if (iconNames != null) icons = getIcons(reg, iconNames);
        else {
    ...
    public void setPrefixedIconNames(String prefix, String... suffixes) {
        ...
        for (int i = 0; i < names.length; i++) names[i] = prefix + "-" + suffixes[i];
        setIconNames(names);
    }
    ...
    IIcon getLocalIcon(int side, int data) {
        if (icons != null) {
            if (side < icons.length) return icons[side];
    ...
    public TE getTileEntity(IBlockAccess world, int x, int y, int z) {
        if (hasTileEntity()) return (TE) world.getTileEntity(x, y, z);
        else return null;
    }
        ...
        if (te instanceof IInventory) {
            ...
            if (var7 != null) {
                for (int var8 = 0; var8 < var7.getSizeInventory(); ++var8) {
                    ...
                    if (var9 != null) scatterNearby(world, x, y, z, var9);
                }
            }
        }
