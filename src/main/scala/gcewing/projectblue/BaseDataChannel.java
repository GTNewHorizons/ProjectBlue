//------------------------------------------------------------------------------------------------
//
//   Mod Base - Data Channel Networking - MC 1.7
//
//------------------------------------------------------------------------------------------------
...
public class BaseDataChannel {
	...
	public static boolean serverDispatch(Object handler, String message, EntityPlayer player, ChannelInput data) {
		if (handler != null) {
			for (Method m : handler.getClass().getMethods()) {
				//System.out.printf("BaseDataChannel.serverDispatch: %s: looking at %s\n", message, m.getName());
				ServerMessageHandler a = m.getAnnotation(ServerMessageHandler.class);
				if (a != null && a.value().equals(message)) {
					try {
						m.invoke(handler, player, data);
					}
					catch (Exception e) {
	...
	public static boolean clientDispatch(Object handler, String message, ChannelInput data) {
		if (handler != null) {
			for (Method m : handler.getClass().getMethods()) {
				ClientMessageHandler a = m.getAnnotation(ClientMessageHandler.class);
				if (a != null && a.value() == message) {
					try {
	...
	static class DataPacket implements ChannelOutput {
		...
		public void close() {
			...
			Packet pkt = new FMLProxyPacket(payload, channel.name);
