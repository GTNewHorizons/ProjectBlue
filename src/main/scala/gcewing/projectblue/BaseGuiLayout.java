    ...
    public static class Grid extends Group {
        ...
        public Grid(int numCols, Object... items) {
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                Widget widget = null;
                if (item != null) {
                    if (item instanceof String) widget = new Label((String) item);
                    else widget = (Widget) item;
                }
                grid[row][col] = widget;
                if (widget != null) add(0, 0, widget);
            }
        }
        ...
        public void layout() {
            ...
            for (int col = 0; col < numCols; col++) {
                if (col > 0) x += colSpacing;
                int w = colWidths[col];
                for (int row = 0; row < numRows; row++) {
                    Widget widget = grid[row][col];
                    if (widget != null) widget.left = x;
                }
                x += w;
            }
            width = x;
            ...
            for (int row = 0; row < numRows; row++) {
                ...
                for (int col = 0; col < numCols; col++) {
                    Widget widget = grid[row][col];
                    if (widget != null) widget.top = y + (h - widget.height) / 2;
                }
                y += h;
            }
            height = y;
