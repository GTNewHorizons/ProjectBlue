...
public class Matrix3 {
    ...
    public static Matrix3[] sideRotations = { /* 0, -Y, DOWN */ ident, /* 1, +Y, UP */ rotX(180),
            /* 2, -Z, NORTH */ rotX(90), /* 3, +Z, SOUTH */ rotX(-90).mul(rotY(180)),
            /* 4, -X, WEST */ rotZ(-90).mul(rotY(90)), /* 5, +X, EAST */ rotZ(90).mul(rotY(-90)) };
    ...
    public Matrix3 mul(Matrix3 n) {
        Matrix3 r = new Matrix3();
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++) r.m[i][j] = m[i][0] * n.m[0][j] + m[i][1] * n.m[1][j] + m[i][2] * n.m[2][j];
        return r;
    }
