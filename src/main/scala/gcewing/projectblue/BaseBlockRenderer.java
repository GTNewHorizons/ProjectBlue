...
public class BaseBlockRenderer<BLOCK extends Block> implements ISimpleBlockRenderingHandler {
	...
	public boolean renderStandardWorldBlock(IBlockAccess world, int x, int y, int z, 
		Block block, int renderID, RenderBlocks rb)
	{
	...
	public static double cubeFaces[][] = {
	...
	void cubeBackFace(Trans3 t, double[] c, double d) {
		...
		backFace(t, c[0] + d * nx, c[1] + d * ny, c[2] + d * nz,
			c[3], c[4], c[5], c[6], c[7], c[8], 0, 0, 16, 16);
	}
	...
	void backFace(Trans3 t,
		double x, double y, double z,
		double dx1, double dy1, double dz1,
		double dx2, double dy2, double dz2,
		double u, double v, double du, double dv)
	{
		face(t, x + dx1, y + dy1, z + dz1,
			-dx1, -dy1, -dz1,
			dx2, dy2, dz2,
			u, v + dv, du, -dv);
	}
	...
	void renderBox(Trans3 t, AxisAlignedBB box, IIcon[] icons) {
		renderBox(t, box.minX, box.minY, box.minZ,
			box.maxX - box.minX, box.maxY - box.minY, box.maxZ - box.minZ, icons);
	}
 ...
 void boxFace(Trans3 t,
		double x, double y, double z,
		double dx1, double dy1, double dz1,
		double dx2, double dy2, double dz2,
		double nx, double ny, double nz,
		double du, double dv, IIcon icon)
	{
		if (icon != null) {
			selectTile(icon);
