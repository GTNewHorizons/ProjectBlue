...
abstract class ItemMultiPartJ extends Item {
    ...
    public boolean onItemUse(ItemStack item, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (d < 1 && place(item, player, world, pos, side, vhit)) return true;
        pos.offset(side);
     ...
     */
    public abstract TMultiPart newPart(ItemStack item, EntityPlayer player, World world, BlockCoord pos, int side,
            Vector3 vhit);
